    ...
    try:
        ...
        for col in numeric_cols:
            if col in df.columns: # Check if the normalized column name exists
                ...
                st.warning(f"Normalized column '{col}' expected for numeric conversion was not found in the DataFrame. (Check your CSV headers and normalization logic).")

        return df
    except FileNotFoundError:
        st.error(f"Error: CSV file not found at {path}. Please ensure the scraper has run and the file exists.")
...
if not df.empty:
    ...
    if 'price_with_vat' in df.columns:
        fig_price_dist = px.histogram(df,
                                      x="price_with_vat", # Use normalized name here
                                      nbins=30,
                                      title="Distribution of Prices with VAT",
                                      template="plotly_white")
        fig_price_dist.update_layout(xaxis_title="Price with VAT (€)", yaxis_title="Number of Flats")
        st.plotly_chart(fig_price_dist, use_container_width=True)
    else:
        st.warning("`price_with_vat` column not found for price distribution chart after normalization.")
    ...
    if 'rooms' in df.columns:
        ...
        if not df['rooms'].dropna().empty:
            ...
            room_counts.columns = ['rooms', 'Count'] # Update column name for the new df too
            fig_rooms = px.bar(room_counts,
                               x="rooms", # Use normalized name here
                               y="Count",
                               title="Number of Rooms Distribution",
                               template="plotly_white")
            fig_rooms.update_layout(xaxis_title="Rooms", yaxis_title="Number of Listings")
            st.plotly_chart(fig_rooms, use_container_width=True)
        else:
            st.warning("`rooms` column found but contains no valid data for room distribution chart.")
    else:
        st.warning("`rooms` column not found for room distribution chart after normalization.")
    ...
    if 'total_area' in df.columns and 'price_with_vat' in df.columns:
        ...
        fig_area_price = px.scatter(plot_df,
                                    ...
                                    template="plotly_white")
        fig_area_price.update_layout(xaxis_title="Total Area (m²)", yaxis_title="Price with VAT (€)")
        st.plotly_chart(fig_area_price, use_container_width=True)
    else:
        st.warning("`total_area` or `price_with_vat` column not found for area vs. price chart after normalization.")
    ...
    if 'status' in df.columns:
        ...
        status_counts.columns = ['status', 'Count'] # Update column name for the new df too
        fig_status = px.pie(status_counts,
                            values='Count',
        ...
        st.warning("`status` column not found for flats by status chart after normalization.")

    # You can add more charts here based on your data and interests!

else:
    st.info("No data available to display charts. Please run the scraper to generate 'ceresne_flats.csv'.")
